...
import { Plus, Search, UserCheck, Phone, Mail, Shield, AlertTriangle, Edit2, Trash2, Eye } from 'lucide-react';
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="table-header">
                ...
                <th className="px-4 py-3 text-left">Psicoflísico</th>
            ...
            <tbody>
              {isLoading ? (
                ...
                const hazBadge = getExpiryBadge(driver.certificadoCargasPeligrosas);
                return (
                  <tr key={driver.id} className="table-row">
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-slate-400">DNI: {driver.dni}</p>
                        </div>
                      </div>
                    </td>
                    <td className="px-4 py-3">
                      {driver.telefono && <p className="flex items-center gap-1 text-xs text-slate-300"><Phone className="w-3 h-3" /> {driver.telefono}</p>}
                      {driver.email && <p className="flex items-center gap-1 text-xs text-slate-400"><Mail className="w-3 h-3" /> {driver.email}</p>}
                    </td>
                    <td className="px-4 py-3">
                      <div>
                        <span className="badge badge-gray text-xs mr-1">{driver.licenciaTipo}</span>
                        <span className={licBadge.cls}>{licBadge.label}</span>
                        <p className="text-xs text-slate-500 mt-0.5">{formatDate(driver.licenciaVencimiento)}</p>
                      </div>
                    </td>
                    <td className="px-4 py-3">
                      <span className={medBadge.cls}>{medBadge.label}</span>
                      <p className="text-xs text-slate-500 mt-0.5">{formatDate(driver.examenMedicoVencimiento)}</p>
                    </td>
                    <td className="px-4 py-3">
                      <span className={psicoBadge.cls}>{psicoBadge.label}</span>
                      <p className="text-xs text-slate-500 mt-0.5">{formatDate(driver.psicofisicoVencimiento)}</p>
                    </td>
                    <td className="px-4 py-3">
                      {driver.habilitadoCargasPeligrosas ? (
                        <div>
                          <span className="badge badge-green text-xs">Habilitado</span>
                          <p className="text-xs text-slate-500 mt-0.5">{formatDate(driver.certificadoCargasPeligrosas)}</p>
                        </div>
                      ) : (
                    ...
                    <td className="px-4 py-3">
                      <span className="text-slate-300">{driver._count?.trips || 0}</span>
                    </td>
                    <td className="px-4 py-3">
                      <div className="flex items-center justify-end gap-1">
                        <button onClick={() => { setEditing(driver); setShowModal(true); }} className="p-1.5 text-slate-400 hover:text-yellow-400 hover:bg-yellow-500/10 rounded transition-colors"><Edit2 className="w-4 h-4" /></button>
                        <button onClick={() => { if (confirm(`¿Eliminar a ${driver.firstName} ${driver.lastName}?`)) deleteMutation.mutate(driver.id); }} className="p-1.5 text-slate-400 hover:text-red-400 hover:bg-red-500/10 rounded transition-colors"><Trash2 className="w-4 h-4" /></button>
                      </div>
                    </td>
                  </tr>
                );
