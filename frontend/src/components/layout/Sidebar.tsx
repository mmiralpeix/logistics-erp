...
export function Sidebar() {
  ...
  return (
    <>
      ...
      <aside className="hidden md:flex w-64 bg-white dark:bg-slate-900 border-r border-slate-200 dark:border-slate-800 flex-col h-screen sticky top-0 overflow-y-auto transition-colors flex-shrink-0">
        {renderNavContent(false)}
      </aside>

      {/* Mobile Drawer (visible only when isMobileMenuOpen is true) */}
      {isMobileMenuOpen && (
        <div className="md:hidden">
          ...
          />
          {/* Slide-over Drawer */}
          <aside className="fixed inset-y-0 left-0 z-50 w-72 bg-white dark:bg-slate-900 border-r border-slate-200 dark:border-slate-800 flex flex-col h-full shadow-2xl">
